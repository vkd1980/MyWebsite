...
$().ready( function() {
		$('#message').hide();
        $("#Account_Code").focus();
        $("#Bill_Date").val($.datepicker.formatDate("yy-mm-dd", new Date()));
        $("#Name").val("CASH");
    ...
    $get_lastID = function() {
        ...
        $newRow = "<tr> \
					<td width='5%' align='centre'><input type='text' readonly='true'  name='slno[]'   id='" + $lastChar + "' value='" + $lastChar + "' class='slnumber form-control'   /></td>\
...
<td width='11%' align='centre'><input type='text' readonly='true' name='Amt[]'id='Amount_" + $lastChar + "'tabindex = '-1' class='amt form-control' style='text-align:right;' /></td>\
...
$("#frmcashbillmaster").validate({
                ...
                submitHandler: function() {
                    var data = $('#frmcashbillmaster').serialize();
    $.ajax({
        url: 'Includes/cash_sales.php',
        type: 'POST',
        ...
        dataType: "json",
		 beforeSend: function() {
            ...
            $('#message').text('Please Wait..... Saving');
        },
        success: function(output) {
            if (output[0] == 'OK' && output[1] == 'OK') {
                $("#message").removeClass('alert-danger');
                $("#message").addClass('alert-success');
                $("#message").fadeIn();
                $('#message').text('Saved Successfully');
				window.open('cashbill.php?action=load&token=<?php echo $_SESSION['token'];?>&finyear=<?php echo $_SESSION['FinYearID']?>&id='+output[2], "popUp", 'directories=no,titlebar=no,toolbar=no,location=no,status=no,menubar=no,scrollbars=no,resizable=no,width=720,height=450');
                setTimeout(function() {
                    location.reload();
                }, 1000);
				/*window.location.replace('cashbill.php?action=load&token=<?php echo $_SESSION['token'];?>&finyear=<?php echo $_SESSION['FinYearID']?>&id='+output[2]);*/
            } else {
                $("#message").fadeIn();
                $("#message").removeClass('alert-success');
                $("#message").addClass('alert-danger');
                $('#message').text('An Error Occured, Check Cash Sale Report before Saving Again');
...
<form name="frmcashbillmaster" id="frmcashbillmaster" method="post"  action="Includes/cash_sales.php" >
...
<input type="hidden" name="token" value="<?php echo $token;?>" />
  ...
  <thead>
    ...
    <th>No</th>
	<th>Date</th>
	<th colspan="2">Paymode</th>
	<th colspan="2">Name</th>
	<th colspan="2">Address</th>
	<th colspan="2">Mob Number</th>
	</tr>
	<tr>
    <th><input type="text" name="Cashbill_No" value="#"  class="form-control"></th>
	<th><input type="text" id="Bill_Date" name="Bill_Date"  class="form-control datepiker" autocomplete="off" /></th>
	<input type="hidden" name="Finyear_ID" value="<?php echo $_SESSION["FinYearID"]; ?>" >
			<input type="hidden" name="Emp_ID" value="<?php echo $_SESSION['userid'];?>" >
	<th colspan="2"><select name="Account_Code" id="AccountCode" class="form-control"  >
	...
	<th colspan="2"><input  type="text" id="Name" name="Name"  class="form-control" autocomplete="off"></th>
	<th colspan="2"><input type="text" id="Address" name="Address"  class="form-control" autocomplete="off" /></th>
	<th colspan="2"><input type="text" id="Mob_No" name="Mob_No" onkeypress=" return isNumberKey(event)" class="form-control" autocomplete="off" /></th>
	</tr>
  </table>
...
<tbody>	
<tr>
<td><input type="text" readonly="true" name="slno[]"   id="1" value="1" class="slnumber form-control"   /></td>
...
<td><input type="text" readonly="true" name="Amt[]" id="Amount_1" tabindex = "-1" class="amt form-control" style="text-align:right;" autocomplete="off" /></td>
<td><input style="visibility:hidden" type='button' class='btn btn-default' /></td>
</tr>
</tbody>
</table>
