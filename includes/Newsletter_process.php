...
if(!empty($_POST['Token']) && (hash_equals($_POST['Token'],hash_hmac('sha256', $_SERVER['SERVER_NAME'].'/includes/Newsletter.php', $_SESSION['csrf_token'])))){
...
if(filter_var($email, FILTER_VALIDATE_EMAIL) === false) {
	...
	$insertSignup= $newsletter->Inssubscription($email,$date,$time);
	 if($insertSignup){
			...
			if(SendMailSmtp('admin@prabhusbooks.com','Admin-Prabhus Books',$email,'Verify Your Email',$message_html,'','')){
			$status = "success";
