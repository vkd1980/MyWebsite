...
switch($action) {
	...
	$count  = mysql_num_rows($query);
		if($count > 0) {
		 ...
		 $responsetext="<div class='panel panel-primary well' ><table class='table table-condensed table-striped table-hover' ><thead class='infilterable'><tr class='alert alert-info'><th>Sno</th>
               	<th>Publihser</th>
              	<th>Actions</th>
	...
	$manufacturers_name = isset($_REQUEST['manufacturers_name']) ? mysql_real_escape_string($_REQUEST['manufacturers_name']) : '';
		...
		$query_customers = "INSERT INTO `manufacturers` (`manufacturers_name`,`date_added`,`last_modified`) VALUES ('" . $manufacturers_name . "', now(),now() )";
	...
	$query = mysql_query("Update `manufacturers` set `manufacturers_name` = '" .$manufacturers_name. "',`last_modified` = now() Where `manufacturers_id`='".$manufacturers_id."' ");
